...
export function render(container) {
  ...
  container.innerHTML = `
    <div class="screen-scroll">
      ...
      <!-- Score Crown -->
      <div class="large-crown-card clickable-card" id="sleep-score-card">
        <div class="lc-wrap">
          <svg class="lc-svg" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="44" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="8"/>
            <circle cx="50" cy="50" r="44" fill="none" stroke="${PALETTE.sleep}" stroke-width="8" stroke-linecap="round"
              stroke-dasharray="198 276.4" stroke-dashoffset="${198 - (198 * score / 100)}" class="arc-animate"/>
          </svg>
          <div class="lc-center">
            <span class="lc-val">${score}</span>
            <span class="lc-lbl" style="color: ${PALETTE.sleep};">Sleep</span>
          </div>
        </div>
        <p class="lc-msg">${getSleepInsight()}</p>
      </div>
